...
def cashAdd(username,password):
	...
	qdb.query_db(qdb.transfer_to_query,final_amount,account_number)
...
def cashRemove(username,password):
	...
	if amount <= int(balance):
		final_amount = int(balance) - amount
		qdb.query_db(qdb.transfer_from_query,final_amount,account_number)
...
def transfercash(username,password):
	...
	__, balance, __, __ = qdb.query_db(qdb.customer_info_query,username,password)[0]
	if receiver_acc_num in account_number:
		...
		if amount_send <= int(balance):
			...
			qdb.query_db(qdb.transfer_from_query,amount_send,receiver_acc_num)
			#qdb.query_db(qdb.transfer_to_query,amount_send,account_number)
